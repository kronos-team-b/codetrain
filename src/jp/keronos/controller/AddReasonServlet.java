...
public class AddReasonServlet extends HttpServlet  {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = DataSourceManager.getConnection()) {
            ...
            inactiveReasonDto.setUserNo(user.getUserNo());
            inactiveReasonDto.setActiveAt(user.getUpdateAt());

            InactiveReasonDao inactiveReasonDao = new InactiveReasonDao(conn);
            inactiveReasonDao.insertReason(inactiveReasonDto);

            String inactiveMessage = user.getLastName() + user.getLastName() + "さんのアカウントを休止しました";
